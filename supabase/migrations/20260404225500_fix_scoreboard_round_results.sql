...
BEGIN
    SELECT current_round INTO v_curr_round FROM public.rooms WHERE id = p_room_id;
    ...
    FOR v_player IN (SELECT * FROM public.players WHERE room_id = p_room_id) LOOP
        ...
        IF v_round_bid > 0 THEN
            IF v_round_tricks >= v_round_bid THEN
               v_points_earned := v_round_bid;
            ELSE
               v_points_earned := -v_round_bid;
            END IF;
            
            v_new_total := v_new_total + v_points_earned;
            UPDATE public.players SET total_score = v_new_total WHERE id = v_player.id;
        END IF;

        -- Record the result in round_results for the scoreboard!
        INSERT INTO public.round_results (room_id, player_id, round_number, bid, tricks_won, points_earned)
        VALUES (p_room_id, v_player.id, v_curr_round, v_round_bid, v_round_tricks, v_points_earned);
    END LOOP;
