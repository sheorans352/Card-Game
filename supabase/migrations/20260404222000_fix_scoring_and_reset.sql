...
BEGIN
    ...
    -- 2. Check for Ultimate Winner (Score >= 250? Set high for now to avoid premature ends unless specified)
    -- Leaving original `31` but maybe you have a different win condition. 
    -- Assuming >= 250 for now based on standard 10x multiplier game rules.
    SELECT id INTO v_winner_id FROM public.players 
    WHERE room_id = p_room_id AND total_score >= 250 
...
CREATE OR REPLACE FUNCTION public.reset_round_data(p_room_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
BEGIN
    -- Clear trick and hand data
    DELETE FROM public.hands WHERE room_id = p_room_id;
    DELETE FROM public.played_cards WHERE room_id = p_room_id;

    -- Reset per-round player counts, absolutely must NULLify bid and flags!
    ...
    -- Set to shuffling for the next round sequence
    UPDATE public.rooms SET 
        status = 'shuffling',
